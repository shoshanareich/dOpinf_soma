...
# Avoid UCX UD transport crashes during MPI_Init on this cluster.
...
ln -sf ${CONFIG_FILE} $scratch_root_dir/config.json
...
#mpiexec -np ${nprocs} python3 $root_dir/src/mixed_rom_forcings.py
#MPI_STATUS=$?
...
RESULTS_DIR="${scratch_root_dir}/${center_opt}_${scale}_${n_days}days_${n_year_train}yrs_${SUFFIX}"
mkdir -p "$RESULTS_DIR"
